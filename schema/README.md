...
| File | Purpose |
|---|---|
| [ao.schema.json](ao.schema.json) | JSON Schema (draft 2020-12) that every record in `data/aos/` is validated against in CI |
| [example.yml](example.yml) | A fully populated reference record. Not real, and deliberately outside `data/` so it never lands in an export |
